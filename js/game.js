...
export class Game {
  ...
  unlock(id) {
    ...
    this.renderPanel();
    if (id === 'browser') {
      this.toast('You unlocked a web browser', 'Reopen it from the Trophies panel. The site inside has one too.');
      this.openBrowser();
    }
  ...
  renderPanel() {
    ...
    if (!this.list) return;
    // The only way into the browser, at every level: break that level's page and
    // open it from its Trophies panel.
